...
void how_to_play() { // 게임 설명 추가하기.
	...
	printf("이 때, 두 닉네임은 동일해서는 안됩니다.(아무키나 입력하세요)\n");
	getch();
	...
	printf("오른쪽 플레이어의 방향키는 IJKL입니다.(아무 키나 입력하세요)\n");
	getch();
...
int game_play() {
	...
	for (; ;)
	{
		...
		if (statusFlag == WIN_FLAG1) {
			...
			double entireTime = difftime(endTime, startTime);
